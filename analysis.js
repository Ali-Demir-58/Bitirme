...
function refreshPortfolioPrices() {
    portfolioData.holdings.forEach(holding => {
        ...
        if (marketFund && marketFund.price) {
            holding.symbol = normalizeSymbol(marketFund.code);
            holding.name = marketFund.name || holding.name;
...
function calculatePortfolioMetrics() {
    ...
    portfolioData.holdings.forEach(holding => {
        ...
        if (marketFund) {
            const fundRisk = Number(marketFund.riskValue || 3);
            weightedVolatility += weight * Math.min(fundRisk, 7);
            weightedMovement += weight * Math.abs(Number(marketFund.daily || 0));
            return;
        }
...
async function saveInvestment() {
    ...
    const assetType = getEl('asset-type')?.value || 'stock';
    ...
    if (assetType === 'gold' || isGoldSymbol(symbol)) {
        ...
        assetName = marketFund?.name || symbol;
    } else {
        const marketStock = findMarketStock(symbol);
        finalSymbol = marketStock?.symbol || symbol;
...
function renderRiskProfileCard() {
    if (!userRiskProfile) {
        safeText('risk-profile-emoji', '📊');
        safeText('risk-profile-type', 'Risk profili bulunamadı');
        safeText('risk-profile-description', 'Risk profilinizi oluşturmak için testi tamamlayabilirsiniz.');
        safeText('risk-profile-score', '-- / 40');
        safeText('risk-profile-date', '--');
        return;
    }
...
function renderRiskProfileAllocation(allocation) {
    const container = getEl('risk-profile-allocation');
